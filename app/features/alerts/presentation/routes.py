...
from app.features.alerts.presentation.depends import get_alerts_repo, get_notifications_history_repo
...
@alerts_router.get("/")
async def get_alerts(workspace_id: str = None, meter_id: str = None, type: AlertType = None,
...
async def get_alerts_notifications(type: AlertType = None, is_read: bool = None, convert_timestamp: bool = False,
                                   user=Depends(verify_access_token),
...
async def get_alert(id: str, user=Depends(verify_access_token),
                    alert_repo: AlertRepository = Depends(get_alerts_repo)
                    ) -> ResponseAlert:
...
@alerts_router.post("/")
async def create_alert(alert_body: AlertCreate, user=Depends(verify_access_token),
                       alert_repo: AlertRepository = Depends(get_alerts_repo)
                       ) -> ResponseAlert:
...
async def update_alert(id: str, alert_body: AlertUpdate, user=Depends(verify_access_token),
                       alert_repo: AlertRepository = Depends(get_alerts_repo)
                       ) -> ResponseAlert:
...
async def delete_alert(id: str, user=Depends(verify_access_token),
                       alert_repo: AlertRepository = Depends(get_alerts_repo)
                       ) -> ResponseAlert:
