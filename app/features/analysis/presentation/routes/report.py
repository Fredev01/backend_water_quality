...
@report_router.post("/{analysis_id}/report/pdf")
async def generate_analysis_pdf_report(
    ...
    try:
        ...
        print(f"Error generating report for analysis {analysis_id}: {e.__class__.__name__}: {e}")
        import traceback
        traceback.print_exc()
...
def _add_average_content(
    ...
    pdf_gen.add_section(
        ReportSection(title="Resultados del Análisis de Promedio", content=None, level=1)
    )
    ...
    if results:
        for sensor_data in results:
            try:
                ...
                pdf_gen.add_chart(chart_image, caption=f"Estadísticas de {sensor_name}", width=90)
            except Exception as e:
...
def _add_average_period_content(
    ...
    pdf_gen.add_section(
        ReportSection(title="Resultados del Análisis de Promedio por Período", content=None, level=1)
    )
...
def _add_single_sensor_period(
    ...
    if averages:
        try:
            ...
            pdf_gen.add_chart(chart_image, caption=f"Tendencia de {sensor} por {period_type_translations.get(period_type, period_type)}")
        except Exception as e:
    ...
    if results:
        for sensor_name, sensor_data in results.items():
            try:
                ...
                chart_image = chart_gen.generate_line_chart(line_data, chart_config)
                pdf_gen.add_chart(chart_image, caption=f"Tendencia de {sensor_name} por {period_type_translations.get(period_type, period_type)}")
            except Exception as e:
...
def _add_prediction_content(
    ...
    pdf_gen.add_section(
        ReportSection(title="Resultados del Análisis de Predicción", content=None, level=1)
    )
...
def _add_single_sensor_prediction(
    ...
    pdf_gen.add_section(
        ReportSection(title=f"Sensor: {sensor}", content=None, level=2)
    )
    ...
    if data and pred:
        try:
            ...
            chart_image = chart_gen.generate_line_chart(line_data, chart_config)
            pdf_gen.add_chart(chart_image, caption=f"Datos históricos y predicciones de {sensor}")
        except Exception as e:
    ...
    for idx, sensor in enumerate(sensors):
        if sensor in data and sensor in pred:
            try:
                ...
                chart_image = chart_gen.generate_line_chart(line_data, chart_config)
                pdf_gen.add_chart(chart_image, caption=f"Predicciones de {sensor.capitalize()}")
...
def _add_correlation_content(
    ...
    pdf_gen.add_section(
        ReportSection(title="Resultados del Análisis de Correlación", content=None, level=1)
    )
    ...
    if matrix and sensors:
        try:
            ...
            pdf_gen.add_chart(chart_image, caption=f"Matriz de correlación usando el método {method_translations.get(method, method)}")
        except Exception as e:
