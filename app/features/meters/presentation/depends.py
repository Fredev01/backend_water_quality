...
from app.features.workspaces.presentation.depends import get_workspace_access
...
def get_water_quality_meter_repo(
    ...
    return WaterQualityMeterRepositoryImpl(
        access=workspace_access)


@lru_cache()
def get_meter_connection(
    water_quality_meter_repo: Annotated[WaterQualityMeterRepository, Depends(get_water_quality_meter_repo)],
) -> WaterQMConnection:

    return WaterQMConnectionImpl(
...
def get_meter_records_repo(
    ...
    return MeterRecordsRepositoryImpl(
